  ...
  switch (this->state_) {
    ...
    case State::DETECTING_WAKE_WORD:
      ...
      if (this->detect_wake_words_()) {
        ...
        if (this->fpc_client_ != nullptr) {
          esp_http_client_cleanup(this->fpc_client_);
          this->fpc_client_ = nullptr;
        }

        const char *url = this->get_upload_url();
        ESP_LOGD("Uploading audio to %s", url);
        ...
        this->fpc_client_ = esp_http_client_init(&config);


        size_t payload_size = this->fpc_ring_buffer_->available();
        ESP_LOGD(TAG, "Uploading %d byte(s)", payload_size);
        this->fpc_ring_buffer_->read((void *)this->fpc_audio_buffer_, payload_size, pdMS_TO_TICKS(200));

        // stringify payload size
        std::stringstream ss;
        ss << payload_size;

        esp_http_client_set_method(this->fpc_client_, HTTP_METHOD_POST);
        esp_http_client_set_header(this->fpc_client_, "Content-Type", "application/octet-stream");
        esp_http_client_set_header(this->fpc_client_, "Content-Length", ss.str().c_str());
        esp_http_client_set_post_field(this->fpc_client_, (const char*)this->fpc_audio_buffer_, payload_size);

        this->set_state_(State::UPLOADING_AUDIO);
      }
      break;
    case State::UPLOADING_AUDIO: {
      esp_err_t err;
      err = esp_http_client_perform(this->fpc_client_);
      ...
      if (err == ESP_OK) {
        ESP_LOGI(TAG, "Successfully uploaded data");
      } else {
        ESP_LOGE(TAG, "Error uploading data: %s", esp_err_to_name(err));
      }

      this->reset_states_();
      this->set_state_(State::DETECTING_WAKE_WORD);
      this->audio_uploaded_trigger_->trigger();
      break;
    }
    case State::STOP_MICROPHONE:
...
size_t MicroWakeWord::read_microphone_() {
  ...
  // Keep audio in case of a false positive
  this->fpc_ring_buffer_->write((void *)this->input_buffer_, bytes_read);
...
bool MicroWakeWord::allocate_buffers_() {
  ...
  if (this->fpc_ring_buffer_ == nullptr) {
    this->fpc_ring_buffer_ = RingBuffer::create(FPC_BUFFER_SIZE * sizeof(int16_t));
    if (this->fpc_ring_buffer_ == nullptr) {
      ESP_LOGE(TAG, "Could not allocate false positive ring buffer");
      return false;
    }
  }

  if (this->fpc_audio_buffer_ == nullptr) {
    this->fpc_audio_buffer_ = audio_samples_allocator.allocate(FPC_BUFFER_SIZE * sizeof(int16_t));
    if (this->fpc_audio_buffer_ == nullptr) {
      ESP_LOGE(TAG, "Could not allocate false positive audio buffer");
      return false;
    }
  }
