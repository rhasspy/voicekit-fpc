...
CONF_ON_AUDIO_UPLOADED = "on_audio_uploaded"
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_ON_AUDIO_UPLOADED): automation.validate_automation(
                single=True
            ),
...
async def to_code(config):
    ...
    if on_audio_uploaded_config := config.get(CONF_ON_AUDIO_UPLOADED):
        await automation.build_automation(
            var.get_audio_uploaded_trigger(),
            [],
            on_audio_uploaded_config,
        )
